...
Two scheduled jobs run **outside this repo**, on the operator's own machine (`~/gokkan-keeper-automation/`, launchd-based, not part of this codebase or its deploys) — implementing the "megatrend candidate pool" investment process described in the README's judgment philosophy:
- **Weekly** (Saturdays): reads the latest Judgment Diary entry whose title contains "메가트렌드 후보군", uses its structured `assets` field (`{type, tickerOrName}[]`) as the ticker list, pulls indicator series via `GET /positions/indicators/series`, and posts a report to Discord via `POST /automation/discord-notify`.
