...
Three scheduled jobs run **outside this repo**, on the operator's own machine (`~/gokkan-keeper-automation/`, launchd-based, not part of this codebase or its deploys):
- **Weekly** (Saturdays): reads the latest Judgment Diary entry whose title contains "메가트렌드 후보군", uses its structured `assets` field (`{type, tickerOrName}[]`) as the ticker list, pulls indicator series via `GET /positions/indicators/series`, and posts a report to Discord via `POST /automation/discord-notify`. Implements the "megatrend candidate pool" investment process from the README's judgment philosophy. Its RSI overbought/oversold thresholds come from `GET /settings` (`weekly_report_rsi_overbought`/`_oversold`, editable on the `/alerts` page) rather than being hardcoded in the prompt.
